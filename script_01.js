...
const firstAndLast = entrepreneurs.map(e => `${e.first} ${e.last}`);

// Pour chaque entrepreneur, remplace la date de naissance par l'âge de l'entrepreneur qu'il aurait aujourd'hui.
const age = entrepreneurs.map(e => `${e.first} ${e.last} is ${new Date().getFullYear() - e.year} years old`);


//Les clés first et last manquent de lisibilité, remplace-les par firstName et lastName
const firstAndLast2 = entrepreneurs.map(e => ({ firstName: e.first, lastName: e.last }));
